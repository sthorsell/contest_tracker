defmodule ContestTracker.Processors.EventProcessor do
  ...
  def hydrate_player(name, pts, "CPT"),
    do: %{name: name, position: "CPT", points: round_points(1.5 * pts)}

  def hydrate_player(name, pts, "FLEX"),
    do: %{name: name, position: "FLEX", points: round_points(pts)}
  ...
  def handle_message(_, message, _) do
    ...
    points_map =
      Enum.reduce(play["statistics"], %{}, fn %{"name" => name, "dk_points" => pts}, res ->
    ...
    |> Task.async_stream(
      fn group ->
        messages =
          Enum.map(group, fn entry ->
            {flexs, cpt} = Enum.split(entry.players, -1)

            lineup =
              Enum.zip([flexs, cpt], ["FLEX", "CPT"])
