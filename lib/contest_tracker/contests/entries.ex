defmodule ContestTracker.Contests.Entries do
  ...
  defp calculate_payout(play_id \\ "36") do
    ...
    query = %{
      query: %{match: %{"play.id.keyword": play_id}},
      size: 0,
      aggs: %{
        points: %{composite: %{size: 10000, sources: [%{points: %{terms: %{field: :points}}}]}}
      }
    }

    {:ok, %{body: %{"aggregations" => %{"points" => %{"buckets" => buckets}}}}} =
      Search.query("entries", "entry", query)

    {_, res} =
      Enum.sort_by(buckets, & &1["key"]["points"], :desc)
  ...
  def get_paginated_entries(play_id \\ "36", page \\ 0) do
    query = %{
      from: page * 10,
      query: %{match: %{"play.id.keyword": play_id}},
      sort: [%{points: %{order: :desc}}, "username.keyword"],
      size: 10
    }
